#!/bin/bash
# A script to distribute an input file to /tmp/${USER}
...
if [ $# -eq 0 ]
then
    echo "Please provide the tar file and run the command as:"
    echo "untar_tmp.bash mytarfile.tar"
    exit 1
fi
...
mpiexec -np $SLURM_TACC_NODES -hosts ${TACC_HOSTLIST} -ppn 1 mkdir -p /tmp/${USER}
[[ ! -z "$realname" ]] &&  mpiexec -np $SLURM_TACC_NODES -hosts ${TACC_HOSTLIST} -ppn 1   tar -xf /tmp/${realname} --directory /tmp/${USER}
echo "Done."
